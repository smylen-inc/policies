...
2. Access is not granted until receipt, review, and approval by the Smylen Security Officer or Privacy Officer.
3. The request for access is retained for future reference.
4. All access to Smylen systems and services is reviewed and updated on a bi-annual basis to ensure proper authorizations are in place commensurate with job functions. The process for conducting reviews is outlined below:
...
3. All Customer support desk interactions must be verified before Smylen support personnel will satisfy any request having information security implications.
   * Smylen's current support desk software, Zendesk, requires users to authenticate before submitting support tickets.
   * Support issues submitted via Smylen's dashboard require that users authenticate with their Smylen account before submitting support tickets.
   * Support issues submitted by email must be verified by Smylen personnel using a phone number that has been registered with the corresponding account.
...
9. All workstations are to have the following messages added to the lock screen and login screen: *This computer is owned by Smylen, Inc. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (https://security-policy.smylen.com) and have completed this training (https://training.smylen.com/). Please contact us if you have problems with this - privacy@smylen.com.*
...
4. Wireless networks managed within Smylen non-production facilities (offices, etc.) are secured with the following configurations:
   ...
   * Passwords are rotated on a regular basis, presently quarterly. This process is managed by the Smylen Security Officer.
...
3. On all production systems and applications in the Smylen environment, password configurations are set to require:
   ...
   * a yearly password expiration for administrative accounts;
...
2. Disallowing transfer of ePHI to workstations is enforced through technical measures.
   * All production access to systems is performed through a bastion/jump host accessed through a VPN. Direct access to production systems is disallowed by Smylen's VPN configuration.
   ...
   * Configuration settings for enforcing these technical controls are managed by Smylen's configuration management tooling.
